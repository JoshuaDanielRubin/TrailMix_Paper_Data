...
plot_props <- function(mcmc_data, output_file_path, source_names, ground_truth = NULL, title_suffix = "", use_log_scale = FALSE) {
  ...
  if (use_log_scale) {
    plot <- plot +
      scale_y_log10(limits = c(1e-12, NA)) +  # Set lower limit to 1e-12
      annotation_logticks(sides = "l")  # Add log-scale tick marks to the left side
  }

  # Add ground truth lines if provided and valid
  if (!is.null(ground_truth) && ground_truth != "") {
    print(ground_truth)
    ground_truth_values <- as.numeric(str_split(ground_truth, ",")[[1]])
    if (length(source_names) == length(ground_truth_values)) {
      ground_truth_df <- data.frame(Haplotype = unique_Haplotypes, Truth = ground_truth_values)
      plot <- plot +
...
ground_truth <- if (length(args) >= 4 && args[4] != "") args[4] else NULL
